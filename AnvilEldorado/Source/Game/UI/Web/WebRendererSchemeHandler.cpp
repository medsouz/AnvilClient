...
WebRendererSchemeHandler::WebRendererSchemeHandler(const std::string &p_Scheme, const std::experimental::filesystem::path &p_Directory, bool p_Main, CefRefPtr<CefFrame> p_Frame)
	: m_Scheme(p_Scheme), m_Main(p_Main), m_Frame(p_Frame)
{
	m_Directory = std::experimental::filesystem::canonical(p_Directory);
	m_RequestedLength = 0;
	m_Offset = 0;
	m_Partial = false;
	m_ShouldCache = false;
...
void WebRendererSchemeHandler::ProcessRequestInternal(CefRefPtr<CefRequest> p_Request, CefRefPtr<CefCallback> p_Callback)
{
	...
	// Get the filename.
	std::experimental::filesystem::path s_Path(s_FinalPath);
...
bool WebRendererSchemeHandler::ReadLocalFile(std::string p_Host, std::string p_Path, std::string& p_OutString)
{
	...
	try
	{
		...
		if (!std::experimental::filesystem::exists(s_FilePath))
			return false;

		// Make sure the file is inside the directory
		s_FilePath = std::experimental::filesystem::canonical(s_FilePath, m_Directory);
		if (!boost::starts_with(s_FilePath, m_Directory))
			return false;

		// using boost's memory mapped file.
		auto s_FileSize = std::experimental::filesystem::file_size(s_FilePath);
